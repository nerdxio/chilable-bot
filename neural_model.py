...
for intent in intents['intents']:
    for pattern in intent['patterns']:
        word_list = nltk.word_tokenize(pattern) # how are you [how,are, you]
...
# Encode the tag labels and convert it to numrcal val to work with in the ml model
label_encoder = LabelEncoder()
y = label_encoder.fit_transform(df['tag'])

# Vectorize the pattern column
#TfidfVectorizer converts the text data into numerical vectors by calculating the term frequency-inverse document frequency (TF-IDF) values
...
# returns the predicted labels for each input pattern.
